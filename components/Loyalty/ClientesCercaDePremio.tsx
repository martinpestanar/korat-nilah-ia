...
import { Target, Gift, Bell, Loader2, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ClientesCercaDePremio: React.FC<ClientesCercaDePremioProps> = ({
    ...
    const totalPages = Math.ceil(totalClientes / maxItems);
    const startIndex = (currentPage - 1) * maxItems;
    const displayedClientes = clientesCercanos.slice(startIndex, startIndex + maxItems);
    ...
    return (
        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm dark:border-dark-border dark:bg-dark-card flex flex-col h-full">
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                {totalClientes > 0 && (
                    <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 self-start sm:self-auto">
            ...
            {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-between border-t border-gray-100 pt-4 dark:border-gray-800">
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                        Mostrando {startIndex + 1}-{Math.min(startIndex + maxItems, totalClientes)} de {totalClientes}
                    </span>
