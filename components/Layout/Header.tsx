...
import { Sun, Moon, Bell, Menu, Check, Bot, AlertTriangle, Info, Sparkles } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
  ...
  // Obtener icono y tooltip según modo
  const getThemeIcon = () => {
    if (mode === 'auto') {
  ...
  const getThemeTooltip = () => {
    if (mode === 'auto') return 'Auto (según hora)';
    if (mode === 'dark') return 'Modo oscuro';
    return 'Modo claro';
  };

  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-light-border bg-light-card px-6 dark:bg-dark-bg dark:border-dark-border transition-colors duration-300">
      {/* Mobile menu trigger */}
      ...
      <div className="ml-auto flex items-center gap-4">
        {/* Theme Toggle con indicador de modo */}
        <button
          onClick={toggleTheme}
        ...
        >
          ...
          {mode === 'auto' && (
            <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-primary ring-2 ring-light-card dark:ring-dark-bg" />
          )}
          {/* Tooltip */}
          <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-dark-bg px-2 py-1 text-xs text-white opacity-0 transition-opacity group-hover:opacity-100 dark:bg-light-card dark:text-dark-bg">
            {getThemeTooltip()}
          </span>
        </button>
