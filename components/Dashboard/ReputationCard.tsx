...
import { Star, MessageSquareQuote, TrendingUp, AlertTriangle } from 'lucide-react';
...
const ReputationCard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const ratedAppointments = appointments.filter(a => a.estado === 'Completada' && a.calificacion > 0);
    const totalRated = ratedAppointments.length;
    
    if (totalRated === 0) return { average: 0, total: 0, recentReviews: [] };

    const sum = ratedAppointments.reduce((acc, curr) => acc + curr.calificacion, 0);
             ...
             stats.recentReviews.map(review => (
                 <div key={review.id} className="rounded-lg bg-gray-50 p-3 dark:bg-[#252525]">
                     <div className="flex items-center justify-between mb-1">
                         <span className="text-xs font-bold text-gray-900 dark:text-white">{review.nombre_cliente}</span>
                         <span className="flex text-yellow-400">
                            {[...Array(review.calificacion)].map((_, i) => <Star key={i} size={8} fill="currentColor" />)}
                         </span>
                     </div>
