...
// Import templates logic in JS
const {
  CATEGORY_DEFAULT_IMAGES,
  CATEGORY_DEFAULT_DESCRIPTIONS,
  SERVICE_TEMPLATES,
  detectCategoryKey,
  resolveServiceMediaAndDesc
} = (() => {
  ...
  const CATEGORY_DEFAULT_DESCRIPTIONS = {
  ...
  function detectCategoryKey(nombre, categoriaNombre) {
    const text = normalizeText(`${nombre} ${categoriaNombre || ''}`);
    if (text.includes('pestaña') || text.includes('pestana') || text.includes('lash') || text.includes('ojo')) return 'pestanas';
    if (text.includes('ceja') || text.includes('brow') || text.includes('laminado')) return 'cejas';
    if (text.includes('uña') || text.includes('una') || text.includes('manic') || text.includes('manos') || text.includes('gel') || text.includes('acrilic') || text.includes('polygel')) return 'manos';
    if (text.includes('pie') || text.includes('pedic')) return 'pies';
    if (text.includes('facial') || text.includes('rostro') || text.includes('cutis') || text.includes('dermaplaning') || text.includes('menton') || text.includes('bozo') || text.includes('labio')) return 'facial';
    if (text.includes('cabello') || text.includes('pelo') || text.includes('balayage') || text.includes('tinte') || text.includes('corte') || text.includes('peinado') || text.includes('alisado') || text.includes('keratina') || text.includes('botox') || text.includes('rizo')) return 'cabello';
    return 'general';
  }

  function resolveServiceMediaAndDesc(nombre, descripcionActual, mediaUrlActual, categoriaNombre) {
    ...
    return { mediaUrl: finalMediaUrl, descripcion: finalDescripcion };
  }

  return { CATEGORY_DEFAULT_IMAGES, CATEGORY_DEFAULT_DESCRIPTIONS, SERVICE_TEMPLATES, detectCategoryKey, resolveServiceMediaAndDesc };
})();

async function populateBrillaStudio() {
  const bId = '10db8ed7-fa79-4092-9bae-760fdad63c75';
  console.log('--- Iniciando enriquecimiento de Brilla Studio ---');
  ...
  const { data: srvs, error: errSrvs } = await supabase.from('carta_servicios').select('*').eq('business_id', bId);
  if (errSrvs) {
    console.error('Error obteniendo servicios:', errSrvs);
    return;
  }

  console.log(`Encontrados ${srvs.length} servicios para procesar...`);

  let updatedCount = 0;
  for (const srv of srvs) {
    const catNombre = catMap[srv.categoria_id] || '';
    const { mediaUrl, descripcion } = resolveServiceMediaAndDesc(srv.nombre, srv.descripcion, srv.media_url, catNombre);

    const updatePayload = {};
    if (!srv.media_url) updatePayload.media_url = mediaUrl;
    if (!srv.descripcion) updatePayload.descripcion = descripcion;
    ...
    if (Object.keys(updatePayload).length > 0) {
      ...
      if (updateErr) {
        console.error(`Error actualizando ${srv.nombre}:`, updateErr.message);
      } else {
        updatedCount++;
      }
    }
  ...
  console.log(`¡Éxito! ${updatedCount} servicios actualizados en Supabase para Brilla Studio.`);
}

populateBrillaStudio();
