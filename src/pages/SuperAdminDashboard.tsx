  ...
  return (
    <div className="h-screen bg-zinc-950 text-white flex overflow-hidden font-sans">
      ...
      `}>
        ...
        <div className="p-5 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-white">God Mode</p>
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Korat Flow Admin</p>
            </div>
          </div>
        </div>
        ...
        {adminInfo && (
          <div className="px-4 py-3 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 flex items-center justify-center text-[11px] font-bold text-white">
                {adminInfo.nombre.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="text-xs font-medium text-zinc-200 truncate">{adminInfo.nombre}</p>
                <p className="text-[10px] text-zinc-500 truncate">{adminInfo.email}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top bar */}
        <header className="h-14 bg-zinc-900/50 border-b border-zinc-800 flex items-center px-4 gap-3 flex-shrink-0">
          <button
          ...
          <div className="flex-1 max-w-sm relative">
            ...
            <input
              ...
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg pl-8 pr-3 py-1.5 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors"
            />
            {searchTerm && (
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all ${refreshing ? 'animate-spin' : ''}`}
              title="Refrescar datos"
            >
              <RefreshCw className="w-4 h-4" />
            </button>
            <div className="px-2 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium">
