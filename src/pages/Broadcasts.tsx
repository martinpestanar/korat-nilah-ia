...
export const Broadcasts: React.FC = () => {
  ...
  // Tabs: 'builder' (Audiencia & Envío) | 'copys' (Biblioteca CRUD) | 'analytics' (Métricas BI)
  const [activeTab, setActiveTab] = useState<'builder' | 'copys' | 'analytics'>('builder');

  // ── Métricas & Analítica de Campañas Masivas
  ...
  const fetchStats = async () => {
    ...
    try {
      const data = await broadcastsApi.getCampaignStats();
      setCampaignStats(data);
    } catch (e) {
      console.warn('Error al cargar analítica de campañas:', e);
    } finally {
  ...
  const previewMessage = useMemo(() => {
    ...
    text = text.replace(/{opt_out}/g, '✨ Si prefieres no recibir más promociones especiales por WhatsApp, responde con la palabra NO.');
    return text;
  ...
  return (
    <div className={`min-h-screen pb-28 font-sans transition-colors duration-300 ${
      ...
      {/* ══════════════════ TAB 3: ANALÍTICA & RESULTADOS BI ══════════════════ */}
      {activeTab === 'analytics' && (
        <div className="px-4 pt-4 space-y-4 max-w-lg mx-auto pb-12">
          {/* Header informativo */}
          <div className={`p-4 rounded-2xl border shadow-xl ${
            isDark ? 'bg-gradient-to-br from-violet-950/40 via-[#0f1422] to-pink-950/30 border-violet-500/30' : 'bg-gradient-to-br from-violet-50 via-white to-pink-50 border-violet-200'
          }`}>
            <div className="flex items-center justify-between mb-2">
              <span className="px-2.5 py-1 rounded-full bg-violet-500/20 text-violet-600 dark:text-violet-300 text-[10px] font-extrabold uppercase tracking-wider flex items-center gap-1">
                <Sparkles className="h-3 w-3" /> Conversional Sales Analytics
              </span>
              <button 
                onClick={fetchStats} 
            ...
            <h3 className={`text-sm font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Medición de Intención & Respuestas
            </h3>
            <p className={`text-xs mt-0.5 leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Clasificación en tiempo real impulsada por IA para medir la tasa de conversión de tus envíos masivos.
            </p>
          </div>

          {loadingStats ? (
            <div className="p-8 text-center space-y-3">
              <RefreshCw className="h-6 w-6 animate-spin text-pink-500 mx-auto" />
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Cargando métricas de campañas...</p>
            </div>
          ) : campaignStats.length === 0 ? (
            <div className={`p-8 text-center rounded-2xl border space-y-3 ${
              isDark ? 'bg-[#0f1422] border-white/10' : 'bg-white border-slate-200'
            }`}>
              <Zap className="h-8 w-8 text-violet-400 mx-auto opacity-60" />
              <p className={`text-xs font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Aún no has realizado envíos masivos registrados</p>
              <p className={`text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Realiza tu primer envío masivo desde la pestaña "Audiencia & Envío" para ver la métrica de intenciones aquí.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {campaignStats.map((camp: any) => {
                const totalObj = camp.clientes_objetivo || camp.mensajes_enviados || 1;
                const resp = camp.respuestas || 0;
                const pos = camp.respuestas_positivas || 0;
                const ind = camp.respuestas_indecisas || 0;
                const neg = camp.respuestas_negativas || 0;
                const sinResp = Math.max(0, totalObj - resp);

                const pctResp = Math.round((resp / totalObj) * 100);
                const pctPos = Math.round((pos / totalObj) * 100);
                const pctInd = Math.round((ind / totalObj) * 100);
                const pctNeg = Math.round((neg / totalObj) * 100);
                const pctSinResp = Math.round((sinResp / totalObj) * 100);

                return (
                  <div key={camp.id} className={`p-4 rounded-2xl border shadow-xl space-y-3 transition-all ${
                    isDark ? 'bg-[#0f1422] border-white/10' : 'bg-white border-slate-200'
                  }`}>
                    {/* Encabezado de la campaña */}
                    <div className="flex items-start justify-between border-b pb-2.5 dark:border-white/5 border-slate-100">
                      <div>
                        <h4 className={`text-xs font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                          {camp.titulo || 'Envío Masivo'}
                        </h4>
                        <p className={`text-[10px] mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                          📅 {new Date(camp.created_at || camp.fecha_envio).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                      <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 text-[10px] font-extrabold">
                        {totalObj} Enviados
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className={`p-2.5 rounded-xl border text-center ${isDark ? 'bg-violet-500/10 border-violet-500/20' : 'bg-violet-50 border-violet-100'}`}>
                        <p className="text-[9px] font-bold text-violet-500 uppercase">Tasa Respuesta</p>
                        <p className={`text-base font-black text-violet-600 dark:text-violet-300 mt-0.5`}>{pctResp}%</p>
                        <p className={`text-[9px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{resp} respuestas</p>
                      </div>
                      <div className={`p-2.5 rounded-xl border text-center ${isDark ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-50 border-emerald-100'}`}>
                        <p className="text-[9px] font-bold text-emerald-500 uppercase">Intención Alta 👑</p>
                        <p className={`text-base font-black text-emerald-600 dark:text-emerald-300 mt-0.5`}>{pctPos}%</p>
                        <p className={`text-[9px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{pos} agendar</p>
                      </div>
                      <div className={`p-2.5 rounded-xl border text-center ${isDark ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-50 border-amber-100'}`}>
                        <p className="text-[9px] font-bold text-amber-500 uppercase">Indecisas 💬</p>
                        <p className={`text-base font-black text-amber-600 dark:text-amber-300 mt-0.5`}>{pctInd}%</p>
                        <p className={`text-[9px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{ind} con duda</p>
                      </div>
                    </div>

                    {/* Desglose visual de Barra Segmentada (Funnel/Donut Representativo) */}
                    <div className="space-y-1.5 pt-1">
                      <p className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        Distribución de Sentimiento de la Clienta:
                      </p>
                      
                      {/* Barra Mixta de Progreso */}
                      <div className="h-3.5 w-full bg-slate-200 dark:bg-black/40 rounded-full overflow-hidden flex">
                        <div style={{ width: `${pctPos}%` }} className="bg-emerald-500 transition-all h-full" title={`Positivas: ${pos}`} />
                        <div style={{ width: `${pctInd}%` }} className="bg-amber-400 transition-all h-full" title={`Indecisas: ${ind}`} />
                        <div style={{ width: `${pctNeg}%` }} className="bg-rose-500 transition-all h-full" title={`Negativas: ${neg}`} />
                        <div style={{ width: `${pctSinResp}%` }} className="bg-slate-400 dark:bg-slate-700 transition-all h-full" title={`Sin Respuesta: ${sinResp}`} />
                      </div>

                      {/* Leyenda Táctil Mobile */}
                      <div className="grid grid-cols-2 gap-2 pt-1 text-[10px]">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 shrink-0" />
                          <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                            🟢 Intención de Agendar: <strong>{pos}</strong> ({pctPos}%)
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full bg-amber-400 shrink-0" />
                          <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                            🟡 Duda / Pregunta: <strong>{ind}</strong> ({pctInd}%)
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full bg-rose-500 shrink-0" />
                          <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                            🔴 Descarte / No le interesa: <strong>{neg}</strong> ({pctNeg}%)
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full bg-slate-400 dark:bg-slate-700 shrink-0" />
                          <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                            ⚪ Sin Respuesta: <strong>{sinResp}</strong> ({pctSinResp}%)
                          </span>
                        </div>
                      </div>
                    </div>
