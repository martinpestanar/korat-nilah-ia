...
BEGIN
    SELECT jsonb_build_object(
        ...
        -- 3. GASTOS, NÓMINA E IMPUESTOS (MÓDULO FINANZAS)
        'salud_financiera_integral', (
            ...
            config_impuestos AS (
                SELECT tax_country, tax_regime, currency
        ...
        -- 9. ESTRATEGIA: MARKETING ACTIVO Y OPORTUNIDADES
        ...
        -- 10. OCUPACIÓN Y CAPACIDAD (NUEVO)
        'ocupacion_y_capacidad', (
            SELECT jsonb_build_object(
                'zonas_muertas_hoy', (
                    SELECT COALESCE(jsonb_agg(hora), '[]'::jsonb)
                    FROM "public"."ocupacion_horaria" 
                    WHERE business_id = p_business_id::text 
                      AND dia_semana = EXTRACT(ISODOW FROM CURRENT_DATE) 
                      AND es_zona_muerta = true
                ),
                'staff_ausente_hoy', (
                    SELECT COUNT(*) 
                    ...
                    WHERE business_id = p_business_id 
                      ...
                      AND tipo IN ('ausencia_todo_el_dia', 'vacaciones')
                ),
                'salon_cerrado_hoy', (
                    SELECT EXISTS(
                        SELECT 1 FROM "public"."dias_cerrados" 
                        WHERE business_id = p_business_id AND fecha = CURRENT_DATE AND es_dia_completo = true
                    )
                )
            )
        ),

        -- 11. RENDIMIENTO DEL COPILOT (NUEVO)
        'rendimiento_copilot', (
            SELECT jsonb_build_object(
                'acciones_hoy', (
                    SELECT COUNT(*) FROM "public"."copilot_action_logs" 
                    WHERE business_id = p_business_id AND created_at >= CURRENT_DATE
                ),
                'errores_hoy', (
                    SELECT COUNT(*) FROM "public"."errores_bot" 
                    WHERE business_id = p_business_id AND created_at >= CURRENT_DATE
                ),
                'canales_activos', (
                    SELECT COALESCE(jsonb_agg(canal), '[]'::jsonb) FROM "public"."chatbot_canales" 
                    WHERE business_id = p_business_id AND activo = true
                )
            )
        ),

        -- 12. FIDELIZACIÓN (NUEVO)
        'fidelizacion_y_premios', (
            SELECT jsonb_build_object(
                'premios_activos', (SELECT COUNT(*) FROM "public"."Premios" WHERE business_id = p_business_id AND activo = true),
                'canjes_este_mes', (
                    SELECT COUNT(*) FROM "public"."canjes" 
                    WHERE business_id = p_business_id AND date_trunc('month', fecha_canje) = date_trunc('month', CURRENT_DATE)
                )
            )
        )
